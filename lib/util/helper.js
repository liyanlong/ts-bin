...
const REGEX = isWin ? /^(.*)\s+(\d+)\s+(\w+)\s*$/ : /^\s*(\d+)\s+(\w+)\s+(.*)\s*/;
...
function findNodeProcess (filterFn) {
    const command = isWin ?
    'wmic Path win32_process Where "Name = \'node.exe\'" Get CommandLine,ProcessId,CSName' :
    ...
    const processList = stdout.toString().split('\n')
        ...
        if (!!line && !line.includes('/bin/sh')) {
            ...
            if (m) {
            const item = isWin ? { user: m[3],pid: m[2], cmd: m[1] } : { user: m[3], pid: m[1], cmd: m[2] };
